...
export default function Dashboard() {
  ...
  const totalAlugueis = alugueisAtivos.reduce(
    (total, contrato) =>
      total + Number(contrato.valor_aluguel || contrato.valor || 0),
    0
  )

  const totalVendas = vendas.reduce(
    (total, contrato) =>
      total + Number(contrato.valor || contrato.valor_venda || 0),
    0
  )

  const totalTemporadas = temporadas.reduce(
    (total, contrato) =>
      total + Number(contrato.valor || contrato.valor_temporada || 0),
    0
  )

  const totalRecebido =
    totalAlugueis + totalVendas + totalTemporadas
    ...
    <main style={{ padding: "30px" }}>
      ...
      >
        ...
        <div className="card">
          ...
          <h2>{alugueisAtivos.length}</h2>
          <p>
            R$ {totalAlugueis.toLocaleString("pt-BR", {
        ...
        <div className="card">
          <h3>Temporadas</h3>
          <h2>{temporadas.length}</h2>
          <p>
            R$ {totalTemporadas.toLocaleString("pt-BR", {
              minimumFractionDigits: 2
            })}
          </p>
        </div>
